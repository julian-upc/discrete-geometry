...
void generate_graph(int n, const VI& S, VVI& adj) {
  adj = VVI(n);
  for (int i = 0; i + 1 < n; ++i) {
    for (int j = i + 1; j < n; ++j) {
      int dif = j - i, dif2 = i - j + n;
      bool valid_edge = false;
      for (VI::const_iterator it = S.begin(); it != S.end() and not valid_edge; ++it)
        if (*it == dif or *it == dif2) valid_edge = true;
      if (valid_edge) {
        adj[i].push_back(j);
        adj[j].push_back(i);
      }
...
int dfs(int u, vector<bool>& seen, int mask, VVI& adj) {
  seen[u] = true;
  int res = 1;
  for (int i = 0; i < int(adj[u].size()); ++i) {
    int v = adj[u][i];
    if (not seen[v] and ((mask >> v)&1) == 0) res += dfs(v, seen, mask, adj);
  }
  return res;
}
...
bool check_balinski(int d, VVI& adj) {
  int n = adj.size();
  ...
  for (int mask = 0; mask < (1 << n); ++mask) {
    ...
    if (bits_set(mask) == d - 1) {
      ...
      if (dfs(start_vertex, seen, mask, adj) != n - d + 1) return false;
    }
  }
  return true;
}
...
bool rec(int act, int u, set<int>& used, vector<PII>& connections, VVI& adj) {
  if (u == connections[act].second) {
    if (act + 1 == connections.size()) return true;
    return rec(act + 1, connections[act + 1].first, used, connections, adj);
  }
  bool found = false;
  for (size_t i = 0; i < adj[u].size() and not found; ++i) {
    const int v = adj[u][i];
    if (used.count(v) and connections[act].second != v) continue;
    used.insert(v);

    if (rec(act, v, used, connections, adj)) found = true;
...
bool psp(int d, VVI& adj) {
  ...
  for (int i = 0; i + 1 < d; ++i) {
    for (int j = i + 1; j < d; ++j) {
      ...
      for (size_t k = 0; k < adj[vertices[i]].size(); ++k) {
  ...
  return connections.size() == 0 ? true : rec(0, connections[0].first, used, connections, adj);
}
...
int main(int argc, char** argv) {
  ...
  //Generate the graph
  VVI adj;
  generate_graph(n, S, adj);
  
  for (int i = 0; i < n; ++i) {
    cerr << i << ": ";
    for (size_t j = 0; j < adj[i].size(); ++j) cerr << adj[i][j] << " ";
    cerr << endl;
  }
  ...
  for (int i = 2; i < n; ++i) {
    if (candidate[i]) candidate[i] = check_balinski(i, adj);    
  }
  
  for (int i = 4; i < n; ++i) if (candidate[i]) candidate[i] = psp(i, adj);
  
  if (candidate[3]) candidate[3] = check_planarity(adj);
