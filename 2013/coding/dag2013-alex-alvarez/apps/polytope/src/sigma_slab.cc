...
perl::Object sigma_slab(int k, int d) {
  if (d < 2)
    throw std::runtime_error("sigma_slab: dimension >= 2 required");
  if (k < 1 || k > d)
    throw std::runtime_error("sigma_slab: 1 <= k <= d required");
  ...
  for (int i = 0; i < k - 1; ++i) {
    for (int j = k; j <= d; ++j) {
      (*v)[0] = 1;
      const Rational lambda(k - 1 - i, j - i);
      for (int t = 0; t < d; ++t) {
        const int u = (i > t ? 1 : 0), w = (j > t ? 1 : 0);
        (*v)[t + 1] = u + lambda*(w - u); 
      }
...
UserFunction4perl("# @category Producing from scratch"
                  "# Produce the Sigma slab, that is the convex hull of the intersection of the hyperplanes \\sum x_i = k and \\sum x_i = k - 1"
